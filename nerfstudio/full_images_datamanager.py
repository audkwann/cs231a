...
class FullImageDatamanager(DataManager, Generic[TDataset]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.config.masks_on_gpu is True:
            self.exclude_batch_keys_from_device.remove("image1_mask")
            self.exclude_batch_keys_from_device.remove("image2_mask")
        if self.config.images_on_gpu is True:
            self.exclude_batch_keys_from_device.remove("image1")
            self.exclude_batch_keys_from_device.remove("image2")
    ...
    def _load_images(
        ...
        def undistort_idx(idx: int) -> Dict[str, torch.Tensor]:
            ...
            # NEW CHANGES TO DO BOTH IMAGES
            for key in ["image1", "image2"]:
                image = data[key]
                assert image.shape[1] == camera.width.item() and image.shape[0] == camera.height.item(), (
                    f'The size of {key} ({image.shape[1]}, {image.shape[0]}) loaded '
                    f'does not match the camera parameters ({camera.width.item(), camera.height.item()})'
                )

                if camera.distortion_params is None or torch.all(camera.distortion_params == 0):
                    continue  # No distortion correction needed for this image

                K = camera.get_intrinsics_matrices().numpy()
                distortion_params = camera.distortion_params.numpy()
                image_np = image.numpy()

                K, undistorted_image, mask = _undistort_image(camera, distortion_params, data, image, K, mask_key=f"{key}_mask")
                data[key] = torch.from_numpy(undistorted_image)
                if mask is not None:
                    data[f"{key}_mask"] = mask

                # Update camera intrinsics and size using first image only (assumes same for both)
                if key == "image1":
                    ...
                    dataset.cameras.width[idx] = undistorted_image.shape[1]
                    dataset.cameras.height[idx] = undistorted_image.shape[0]
        ...
        # Move to device.
        # NEW CHANGES TO HAVE BOTH IMAGES
        for cache in undistorted_images:
            for key in ["image1", "image2"]:
                if cache_images_device == "gpu":
                    cache[key] = cache[key].to(self.device)
                    if f"{key}_mask" in cache:
                        cache[f"{key}_mask"] = cache[f"{key}_mask"].to(self.device)
                elif cache_images_device == "cpu":
                    cache[key] = cache[key].pin_memory()
                    if f"{key}_mask" in cache:
                        cache[f"{key}_mask"] = cache[f"{key}_mask"].pin_memory()
                else:
    ...
    def fixed_indices_eval_dataloader(self) -> List[Tuple[Cameras, Dict]]:
        ...
        """
        # MODIFIED TO INCORPORATE TWO IMAGES
        ...
        for i in image_indices:
            data[i]["image1"] = data[i]["image1"].to(self.device)
            data[i]["image2"] = data[i]["image2"].to(self.device)
    ...
    def get_train_rays_per_batch(self):
        ...
        if len(self.cached_train) != 0:
            h = self.cached_train[0]["image1"].shape[0]
            w = self.cached_train[0]["image1"].shape[1]
            return h * w
        else:
            return 800 * 800
    ...
    def next_train(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        # MODIFIED TO INCORPORATE TWO IMAGES
        data["image1"] = data["image1"].to(self.device)
        data["image2"] = data["image2"].to(self.device)
...
# NEW CHANGES: take in mask_key and changed "if mask_key in data:"
def _undistort_image(
    ...
    if camera.camera_type.item() == CameraType.PERSPECTIVE.value:
        ...
        # CHANGED TO MASK_KEY INSTEAD OF MASK
        if mask_key in data:
