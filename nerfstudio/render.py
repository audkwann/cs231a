# MAY 11 UPDATES: added batch=batch as args to get_outputs and get_outputs_for_camera
    ...
    with ExitStack() as stack:
        ...
        with progress:
            for camera_idx in progress.track(range(cameras.size), description=""):
                ...
                batch = dataset.get_data(camera_idx, image_type=self.cache_images_type)
                ...
                if render_nearest_camera:
                    ...
                    for i in range(len(train_cameras)):
                        ...
                        # NEW CHANGES (following line)
                        outputs = pipeline.model.get_outputs(ray_bundle, batch=batch)
                ...
                # NEW: added batch=batch to get_outputs_for_camera
                if crop_data is not None:
                    with renderers.background_color_override_context(
                        crop_data.background_color.to(pipeline.device)
                    ), torch.no_grad():
                        outputs = pipeline.model.get_outputs_for_camera(
                            cameras[camera_idx : camera_idx + 1], batch=batch, obb_box=obb_box
                        )
                else:
                    with torch.no_grad():
                        outputs = pipeline.model.get_outputs_for_camera(
        ...
        for split in self.split.split("+"):
            ...
            with Progress(
                ...
                for camera_idx, (camera, batch) in enumerate(progress.track(dataloader, total=len(dataset))):
                    with torch.no_grad():
                        outputs = pipeline.model.get_outputs_for_camera(camera, batch=batch)
